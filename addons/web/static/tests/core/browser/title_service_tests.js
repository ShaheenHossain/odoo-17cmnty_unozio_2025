...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Unozio" });
    assert.strictEqual(env.services.title.current, "Unozio");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Unozio", chat: null });
    assert.strictEqual(env.services.title.current, "Unozio");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Unozio - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Unozio", action: "Import" });
    assert.strictEqual(env.services.title.current, "Unozio - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Unozio", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Unozio - Import"); // parts is a copy!
});
