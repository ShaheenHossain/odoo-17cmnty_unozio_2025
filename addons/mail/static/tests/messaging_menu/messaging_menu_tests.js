...
QUnit.test("rendering with UnozioBot has a request (default)", async (assert) => {
    ...
    await contains(".o-mail-NotificationItem", { text: "UnozioBot has a request" });
});

QUnit.test("rendering without UnozioBot has a request (denied)", async () => {
    patchBrowserNotification("denied");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
...
QUnit.test("rendering without UnozioBot has a request (accepted)", async () => {
